...
const ProfileCard = ({
  ...
  return <motion.div initial={{
    ...
    opacity: 0
  }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={onClose}>
      <motion.div initial={{
      scale: 0.9,
      opacity: 0
    }} animate={{
      ...
      scale: 0.9,
      opacity: 0
    }} className="glass rounded-3xl overflow-hidden shadow-card w-full max-w-sm" onClick={e => e.stopPropagation()}>
        {/* Close button */}
        <button onClick={onClose} className="absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-black/50 flex items-center justify-center">
          <X className="w-5 h-5 text-white" />
        </button>

        {/* Image */}
        <div className="relative">
          <img src={match.avatar} alt={match.name} className="w-full h-64 object-cover" />
        ...
        <div className="p-5">
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {match.interests.map(interest => <span key={interest} className="px-3 py-1 bg-muted rounded-full text-xs font-medium text-muted-foreground">
                {interest}
              </span>)}
          </div>
          ...
          <div className="flex gap-3">
            ...
            <Button variant="glow" className="flex-1" onClick={handleMessage}>
...
const MosaicTile = ({
  ...
  return <motion.div initial={{
    ...
    delay: index * 0.05
  }} className="relative aspect-square cursor-pointer overflow-hidden rounded-lg group" onClick={onClick}>
      <img src={match.avatar} alt={match.name} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
      {/* Overlay on hover */}
...
export const MatchesScreen = () => {
  ...
  return <div className="min-h-screen pb-24 px-0 py-0 flex flex-col">
      ...
      <AnimatePresence>
        {selectedMatch && <ProfileCard match={selectedMatch} onClose={() => setSelectedMatch(null)} />}
      </AnimatePresence>
